...
Proof.
  ...
  - pose proof me_stack_localstack_aligned (me_safe_cm_env h_match_env) as h_align.
    ...
    red.
    !intros.
    specialize (h_align _ h_lt_δ_lvl).
  ...
  - !invclear h_CM_eval_expr_load_id_v.
    ...
    + destruct vaddr;simpl in *;try discriminate.
      ...
      * !assert (lvl < Datatypes.length CE)%nat.
        { omega. }
        !destruct (h_aligned_g_m _ h_lt_lvl0).
...
Proof.
  ...
  - !invclear h_CM_eval_expr_load_id_v.
    ...
    + destruct vaddr;simpl in *;try discriminate.
      ...
      !!pose proof h_aligned_g_m lvl h_lt_lvl0.
      decomp h_ex.
...
Proof.
  ...
  !intros.
  specialize (h_aligned_g_m (δ_lvl) h_lt_δ_lvl).
  ...
  - cbn.
    ...
    specialize (h_aligned_g_m0 δ_lvl h_lt_δ_lvl0).
...
Proof.
  ...
  - specialize (IHn'' (S n') m sp).
    ...
    decomp IHn''.
    !!pose proof chained_stack_structure_decomp_S_2 n' m sp' h_chain_sp' g e v h_CM_eval_expr_v0.
    decomp h_ex.
(*     rename x into sp_Sn''. *)
    exists sp'0;split;[|split];eauto.
    + cbn.
      econstructor.
      * eassumption.
...
Proof.
  ...
  - !!destruct CE eqn:heq_CE.
    ...
    + rename l into CE'.
      cbn in *.
      subst.
      specialize (IHh_chain_n_stkptr (Datatypes.length CE') CE' locenv g eq_refl).
      !!assert (Datatypes.length CE' ≤ n) by omega.
      specialize (IHh_chain_n_stkptr h_le).
      destruct δ_lvl.
      * cbn.
        exists b.
        constructor.
        ...
        reflexivity.
      * !!assert (δ_lvl < Datatypes.length CE')%nat by omega.
        specialize (IHh_chain_n_stkptr δ_lvl h_lt_δ_lvl0).
        decomp IHh_chain_n_stkptr.
        exists b_δ.
        change (S δ_lvl) with (1 + δ_lvl)%nat.
        !assert (chained_stack_structure m (1 + δ_lvl) (Values.Vptr b Int.zero)).
        { econstructor.
          eapply chained_stack_structure_le;eauto;omega.
          cbn.
          assumption. }
        edestruct chain_structure_cut with (1:=h_chain).
        econstructor.
...
(* The forbidden *addresses* remain the same when storing values of
   parameters in the local stack.  *)
Lemma exec_init_locals_preserve_forbidden:
...
Proof.
  ...
  - rename x1 into objname_t.
    ...
    + !intros.
      ...
      * red;!intros.
        red in H.
        specialize (fun spb_id ofs_id0 => H _ _ _ spb_id ofs_id0 heq_transl_variable heq0).
        specialize (H spb_id ofs_id0).
        ...
        { !assert (Cminor.eval_expr g sp e_chain m' id_t (Values.Vptr spb_id ofs_id0)).
          ...
          pose proof (chain_struct_build_loads_ofs _ _ _ H1 n _ _ _ _ h_CM_eval_expr_objname_t_objname_t_v) .
          subst;assumption. }
...
Lemma init_locals_unchanged_on:
  forall decl st CE locvarinit,
    init_locals st CE decl =: locvarinit ->
...
Proof.
  ...
  rewrite function_utils.build_frame_decl_ok in H2.
  destruct x.
...
Lemma add_to_frame_sz: forall stbl fram_sz parname parsubtype p sz,
    ...
    -> spark2Cminor.compute_size stbl parsubtype = OK sz
