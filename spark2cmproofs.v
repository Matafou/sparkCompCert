...
Definition stack_localstack_aligned (CE:compilenv) locenv g m := forall b δ_lvl v,
    ...
    Cminor.eval_expr g b locenv m (build_loads_ (Econst (Oaddrstack Integers.Int.zero)) δ_lvl) v ->
...
Proof.
  ...
  - cbn in *.
    destruct i₂;auto.
    apply occur_build_loads in heq_build_loads.
    exfalso;omega.
  - destruct i₂.
...
(* Addresses that should be untouched by a function call: the one
   ...
   can be changed. In the future this may also include things
   allocated during the function called, and alsoamybe local variables
   (not in stack). Moreover at the end of the function call, the frame
   used for it was indeed free at call time, so forbidden mcaller
   mcallee is strictly included in forbidden m m. *)
...
(* A stricter notion for procedure body, where the chaining arg does not change. *)
...
Proof.
  ...
  constructor;cbn.
  f_equal.
  eapply chained_stack_struct_sp_add;eauto.
Qed.
...
Lemma chained_stack_structure_decomp_S m sp : forall n,
    chained_stack_structure m (S n) sp ->
    forall g e v,
      Cminor.eval_expr g sp e m (Eload AST.Mint32 (Econst (Oaddrstack Int.zero))) v ->
      exists sp',
        Cminor.eval_expr g sp e m (Eload AST.Mint32 (Econst (Oaddrstack Int.zero))) sp' /\
        Cminor.eval_expr g sp' e m (Econst (Oaddrstack Int.zero)) v.
Proof.
  !intros.
  pose proof (chained_stack_structure_aux _ _ _ H g e).
  decomp H0.
  exists (Values.Vptr x Int.zero);split;auto.
  !invclear h_CM_eval_expr_v;subst.
  constructor.
  !invclear h_CM_eval_expr_vaddr;subst.
  cbn in *.
  !invclear h_eval_constant.
  rewrite <- heq0.
  rewrite Int.add_zero.  
  rewrite <- heq.
  destruct sp;try discriminate;cbn.
  rewrite Int.add_zero.  
  reflexivity.
Qed.
...
Proof.
  ...
  - !intros.
    ...
    change (Eload AST.Mint32 (build_loads_ (Econst (Oaddrstack Int.zero)) n)) with (build_loads_ (Econst (Oaddrstack Int.zero)) (S n)) in H2.
    assert (chained_stack_structure m (S n) sp).
    { eapply chained_stack_structure_le;eauto. }
    specialize (IHn m sp H0 _ _ _ H2).
    decomp IHn.
    exists x;split;auto.
    cbn.
    econstructor;eauto.
...
Proof.
  ...
  pose proof chained_stack_structure_decomp_S_2 _ _ _ H g e v h_CM_eval_expr_v.
  decomp H0.
  ...
  rewrite Nat.add_comm.
  assumption.
Qed.  xxxxxxx This should help.

(*
Lemma chained_stack_structure_decomp_S_3: forall n' n n'' m sp,
    (n > n' + n'')%nat -> 
    chained_stack_structure m n sp ->
    forall g e v,
      Cminor.eval_expr g sp e m (build_loads_ (build_loads_ (Econst (Oaddrstack Int.zero)) n'') (S n')) v ->
      exists sp',
        Cminor.eval_expr g sp e m (Eload AST.Mint32 (Econst (Oaddrstack Int.zero))) sp' /\
        Cminor.eval_expr g sp' e m (build_loads_ (build_loads_ (Econst (Oaddrstack Int.zero)) n'') n') v.
Proof.
  intro n'.
  !induction n'.
  - !intros.
...
Lemma chained_stack_structure_eval: forall n m sp,
    chained_stack_structure m (S n) sp ->
    forall g e,
    exists v,
      Cminor.eval_expr g sp e m (build_loads_ (build_loads_ (Econst (Oaddrstack Int.zero)) n) (S n)) v.
Proof.
  intro n.
  change (Econst (Oaddrstack Int.zero)) with (build_loads_ (Econst (Oaddrstack Int.zero)) 0).
  intros m sp H g e. 
  ...
  - !inversion H;subst.
    specialize (IHn m (Values.Vptr b' Int.zero) H1 g e).
    decomp IHn.
    exists x.
    set (CM0:= (Econst (Oaddrstack Int.zero))) in *.
    
    up_type.
    change (S (S n)) with (1 + S n)%nat.
    rewrite build_loads_compos.
    cbn.
    eapply eval_Eload with (vaddr:=(Values.Vptr b' Int.zero)).
    + econstructor;eauto.
    econstructor;eauto.
...
Lemma chained_stack_structure_decomp_S_2_rev: forall n m sp,
    chained_stack_structure m (S n) sp ->
    forall g e v,
    forall sp',
      Cminor.eval_expr g sp e m (Eload AST.Mint32 (Econst (Oaddrstack Int.zero))) sp' ->
      Cminor.eval_expr g sp' e m (build_loads_ n) v ->
      Cminor.eval_expr g sp e m (build_loads_ (S n)) v.
Proof.
  intro n.
  !induction n;!intros.
  - cbn in *.
    ...
    econstructor;eauto.
    !inversion h_CM_eval_expr_v;subst;cbn in *.
    pose proof chained_stack_structure_aux _ _ _ H g e.
    decomp H0.
    assert (sp' = (Values.Vptr x Int.zero)).
    { eapply det_eval_expr;eauto. }
    subst.
    !!erewrite chained_stack_struct_sp_add in h_eval_constant;eauto.
    inversion h_eval_constant;subst;auto.
  - assert (h_chain:chained_stack_structure m (S n) sp').
    { pose proof chained_stack_structure_aux _ _ _ H g e.
      decomp H0.
      assert (sp' = (Values.Vptr x Int.zero)).
      { eapply det_eval_expr;eauto. }
      subst.
      assumption. }
    specialize (IHn m sp' h_chain g e).
    
    cbn in h_CM_eval_expr_v.
    inversion h_CM_eval_expr_v;subst.


    change (Eload AST.Mint32 (build_loads_ n)) with (build_loads_ (S n)) in H2.
    assert (chained_stack_structure m (S n) sp).
    { eapply chained_stack_structure_le;eauto. }
    specialize (IHn m sp H0 _ _ _ H2).
    decomp IHn.
...
Proof.  
  ...
  - cbn in *.
    ...
    constructor;cbn.
    erewrite chained_stack_struct_sp_add;eauto.
  - cbn in h_CM_eval_expr_v.
    !invclear h_CM_eval_expr_v.

    rewrite plus_Snm_nSm in H.
    rewrite <- build_loads_compos_comm in h_CM_eval_expr_v.
    rewrite <- plus_Snm_nSm in h_CM_eval_expr_v.
    rewrite build_loads_compos_comm in h_CM_eval_expr_v.
    pose proof (IHn' (S n'') m sp g e v H h_CM_eval_expr_v) as IHn'_1.
    decomp IHn'_1.
    
    

    assert (chained_stack_structure m (n' + n'') sp).
    { rewrite <- plus_Snm_nSm in H.
      cbn in H.
      eapply chained_stack_structure_le;eauto. }    
    !assert (exists sp'', Cminor.eval_expr g sp e m (build_loads_ (build_loads_ (Econst (Oaddrstack Int.zero)) n'') n') sp'').
    { admit. }
    !!destruct h_ex as [sp'' ?].
    pose proof IHn' _ _ _ _ _ _ H0 h_CM_eval_expr_sp''.
    decomp H1.
    ...
    pose proof chained_stack_structure_decomp_S_2 _ _ _ H g e _ h_CM_eval_expr_v.
    cbn in *.
    subst.
    pose proof chained_stack_structure_decomp_S_2 _ _ _ H g e _ h_CM_eval_expr_v.
    decomp H0.
    ...
    !!assert (h_chain_x:chained_stack_structure m (n' + n'') x). { admit. (* lemma *) }
    specialize (IHn' n'' (n' + n'')%nat m x h_chain_x g e v h_CM_eval_expr_v0 eq_refl).
    up_type.
    decomp IHn'.
...
Lemma chained_stack_structure_decomp_S_n:
  forall n' n'',
  forall n m sp,
    chained_stack_structure m n sp ->
    forall g e v,
      ...
      Cminor.eval_expr g sp e m (build_loads_ (Econst (Oaddrstack Int.zero)) n) v ->
        exists sp',
          Cminor.eval_expr g sp e m (build_loads_ (Econst (Oaddrstack Int.zero)) n') sp' /\
          Cminor.eval_expr g sp' e m (build_loads_ (Econst (Oaddrstack Int.zero)) n'') v. (* change e to some e' *)
Proof.
  !intros.
  subst.
  rewrite build_loads_compos in h_CM_eval_expr_v.
  ...
  - cbn in *.
    ...
    xxx (* case analysis on n' ?a *)
    inversion h_CM_eval_expr_x0.
    eapply eval_Eload with (vaddr:=x).
    eauto.
    ...
    { admit. (* lemma *) }
    pose proof chained_stack_structure_aux _ _ _ H2 g e.
    decomp H3.
...
Proof.
  ...
  - unfold build_loads in *.
    cbn in *.
    !inversion h_CM_eval_expr;subst;clear h_CM_eval_expr.
    !inversion h_CM_eval_expr_v1;subst;clear h_CM_eval_expr_v1.
    !inversion h_CM_eval_expr_v2;subst; clear h_CM_eval_expr_v2.
    rewrite <- eval_constant_ok in h_eval_constant, h_eval_constant0.
    functional inversion h_eval_constant;subst;clear h_eval_constant.
    functional inversion h_eval_constant0;subst;clear h_eval_constant0.
    cbn in *.
...
Proof.
  ...
  - specialize (IHr x0 heq).
    ...
    + rename m1 into m_step.
      rename e1 into e_step.
      rename x0 into initparams'.
      up_type.
      apply trans_unchanged with m_step.
      * !inversion h_exec_stmt;subst.
        unfold Mem.storev in heq_storev_v_m_step.
        destruct x1_v;try discriminate.
        apply Mem.store_unchanged_on with (1:=heq_storev_v_m_step).
        !intros.
      ...
      * assert (Mem.unchanged_on (forbidden st CE g astnum e_step sp m_step m_step) m_step m').
        { eapply IHr;eauto. }
        clear IHr.
        ...
        Lemma forbidden_exec_initparam_iff:
          forall s g proc_t sp e m_before chnk addr t1 m_after var st CE astnum v,
            exact_levelG CE ->
            stack_localstack_aligned CE e g m_before ->
            match_env st s CE sp e g m_before  ->
            transl_variable st CE astnum var =: addr ->
            exec_stmt g proc_t sp e m_before 
                      (Sstore chnk addr v) t1 e m_after Cminor.Out_normal ->
            forall v, Cminor.eval_expr g sp e m_before addr v -> Cminor.eval_expr g sp e m_after addr v.
        Proof.
          !intros.
          !inversion h_exec_stmt;subst;eq_same_clear;clear h_exec_stmt;up_type.
          !!pose proof (h_match_env.(me_stack_no_null_offset)).
          specialize (h_nonul_ofs astnum var).
          rewrite heq_transl_variable in h_nonul_ofs.
          !functional inversion heq_transl_variable.
          rename m' into lvl_top_CE.
          rewrite <- heq_build_loads in h_nonul_ofs.
          remember (lvl_top_CE - lvl_var)%nat as δ_lvl_var.
          specialize (h_nonul_ofs _ _ eq_refl).
          rewrite <- cm_storev_ok in heq_storev_v_v_m_after.
          functional inversion heq_storev_v_v_m_after.
          try rewrite cm_storev_ok in *.
          subst;up_type.
(*           assert (ofs = Int.repr δ_var). *)
(*           { admit. } *)
          subst.
          eapply wf_chain_load_var;eauto.
          eapply wf_chain_load_aligned;eauto.
              ...
              rewrite Int.add_unsigned at 1.
              rewrite Int.repr_unsigned.

              repeat match goal with
              | H: ?e = ?e |- _ => clear H
              end.
k              rewrite <- add_val_ok in heq0.
              functional inversion heq0;subst.
              eq_same_clear.
            ...
            simpl in heq.
            !inversion heq;clear heq.
            destruct v1; destruct v2; simpl in heq0;try discriminate.
          Qed.
        ...
        Lemma forbidden_exec_initparam_iff:
          forall s g proc_t sp e m_before chnk addr id t1 m_after var st CE astnum,
             exact_levelG CE ->
             stack_localstack_aligned CE e g m_before ->
             match_env st s CE sp e g m_before  ->
            transl_variable st CE astnum var =: addr ->
            exec_stmt g proc_t sp e m_before 
                      (Sstore chnk addr (Evar (transl_paramid id))) t1 e m_after Cminor.Out_normal ->
            forall a b, invisible_cminor_addr st CE g astnum e sp m_before a b
                        -> invisible_cminor_addr st CE g astnum e sp m_after a b.
        Proof.
          !intros.
          !inversion h_exec_stmt;subst;eq_same_clear;clear h_exec_stmt;up_type.
          !!pose proof (h_match_env.(me_stack_no_null_offset)).
          ...
          assert (Cminor.eval_expr g sp e m_after var_t var_t_v).
          { rewrite <- cm_storev_ok in heq_storev_v_m_after.
            functional inversion heq_storev_v_m_after.
            try rewrite cm_storev_ok in *.
            subst.
            assert (ofs = Int.repr δ_var).
            { admit. }
            ...
            with (1:= h_exact_lvlG_CE) (2:=heq_storev_v_m_after).
          ...
          { induction δ_lvl_var.
            - unfold build_loads in heq_build_loads.
              cbn in *.
              subst.
              inversion h_CM_eval_expr_var_t_var_t_v;subst.
              
              

          }

          !!remember (m'0 - lvl_id)%nat as δ.
          induction δ.
          - unfold build_loads in heq_build_loads;simpl in heq_build_loads.
            subst.
            inversion h_CM_eval_expr_id_t_id_t_v;subst.
            split;intros [h_forbidden_1 h_forbidden_2];red;split.
            
  - split.
    + red;!!intros.
      red in h_forbidden_1.
      eapply h_forbidden_1;eauto;clear h_forbidden_1 h_forbidden_2.
      
      edestruct h_stk_mtch_s'_m'.
  
  

        
        Lemma forbidden_exec_initparam_iff:
          forall s s'  e_v e_t_v g proc_t sp e m_before x x1 id t1 m_after st CE astnum,
            match_env st s CE sp e g m_before  ->
            exact_levelG CE ->
            all_frm_increasing CE ->
            stack_match st s CE sp e g m_before ->
            transl_value e_v e_t_v ->
            storeUpdate st s (E_Identifier astnum id) e_v (Normal s') ->
            exec_stmt g proc_t sp e m_before 
                      (Sstore x x1 (Evar (transl_paramid id))) t1 e m_after Cminor.Out_normal ->
            forall a b, (forbidden st CE g astnum e sp m_before m_before a b
                         <-> forbidden st CE g astnum e sp m_after m_after a b).
        Proof.
          !intros.
          !inversion h_exec_stmt.
          assert (stack_match st s' CE sp e g m_after).
          ...
          split;intros [h_forbidden_1 h_forbidden_2];red.

          - split.
            + red;!!intros.
              red in h_forbidden_1.
              eapply h_forbidden_1;eauto;clear h_forbidden_1 h_forbidden_2.
              assert (stack_match st s' CE sp e_step g m_step).
              { eapply assignment_preserve_stack_match;eauto.  with (9:=h_exact_lvlG_CE).
                
                eapply assignment_preserve_stack_match with (1:= h_exact_lvlG_CE) (2:=h_allincr_CE) (3:=heq_transl_variable)
                                                                                  (4:=heq_transl_value_e_v_e_t_v);eauto.

.

                up_type.
              pose proof (assignment_preserve_stack_match _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _
                                                          h_exact_lvlG_CE h_allincr_CE heq_transl_variable
                                                          heq_transl_value_e_v_e_t_v
                         ).
  ...
  - rename x1 into chaining_arg.
    ...
    + !assert (
      ...
      * (* RENAMING *)
        ...
        !assert (Mem.unchanged_on (forbidden st CE g astnum e sp m_chain m_chain) m_chain m_init_params).
        {
...
Proof.
  ...
  - rename x1 into chaining_expr.
    ...
    { erewrite (cut_until_exact_levelG _ _ _ _ _ _ h_CEcut_CE_pb_lvl).
      reflexivity. }
